...
import { STATUS_FETCH } from "./status.enum";
...
export const getQuoteAsync = createAsyncThunk("quote/getQuote", async (personagem: string) => {
  try {
...
export const getQuoteFromAPI = (personagem: string) => (dispatch: AppDispatch) => {
  dispatch(limpar());
